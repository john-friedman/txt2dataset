...
results, errors = builder.build(prompt=prompt, schema=DividendExtraction, model="gpt-4o-mini",
               entries=entries, rpm=500, tpm=200_000, rpm_threshold=0.75, tpm_threshold=0.75)
print(results)

spotchecks = builder.spotcheck(prompt=prompt, schema=DividendExtraction, model="gpt-4o-mini", entries=entries,
               results=results, sample_size=10, rpm=500, tpm=200_000, rpm_threshold=0.75, tpm_threshold=0.75)
print(spotchecks)

builder.spotcheck_visualize(prompt=prompt, schema=DividendExtraction, model="gpt-4o-mini", entries=entries,
               results=results, sample_size=10, rpm=500, tpm=200_000, rpm_threshold=0.75, tpm_threshold=0.75)
